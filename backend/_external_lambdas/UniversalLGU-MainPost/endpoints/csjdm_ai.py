"""CSJDM AI chat endpoint — San Jose Del Monte branch.

Gemini-backed LGU services assistant. Mirrors the atty_tony_chat / cebu_ai_chat
contract so the Flutter client can reuse the same request/response shape.
"""
import json
import logging
from helpers.auth import ok, fail, require
from helpers.csjdm_ai import chat_csjdm_ai

logger = logging.getLogger()


def csjdm_ai_chat(cur, data, files, ts):
    """Turn-based chat with the CSJDM LGU assistant.
    ...
    try:
        logger.info(
            f"csjdm_ai_chat called | keys={list(data.keys())} "
        ...
        reply = chat_csjdm_ai(history, lang=lang)
        return ok({'status': True, 'reply': reply})
    except ValueError as e:
        return fail(str(e))
    except Exception as e:
        logger.error(f"csjdm_ai_chat error: {e}", exc_info=True)
